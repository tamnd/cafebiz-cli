...
describes what users get. Pick a format with `-o`, or let cafebiz choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cafebiz <command> -o table   # aligned columns for reading
cafebiz <command> -o jsonl   # one JSON object per line, for piping
cafebiz <command> -o json    # a single JSON array
cafebiz <command> -o csv     # spreadsheet friendly
cafebiz <command> -o tsv     # tab-separated
cafebiz <command> -o url     # just the URL column
cafebiz <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cafebiz <command> --fields id,title,url
```
...
```bash
cafebiz <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
cafebiz <command>            # a table, because this is a terminal
cafebiz <command> | wc -l    # JSONL, because this is a pipe
```
