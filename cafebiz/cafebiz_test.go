...
func sampleRSS(n int) string {
	...
	for i := 0; i < n; i++ {
		items += fmt.Sprintf(`
		<item>
			<title>Doanh nhân thành công số %d năm 2026</title>
			<link>https://cafebiz.vn/doanh-nhan-thanh-cong-so-%d-1900%02d.chn</link>
			<description>Mô tả về doanh nhân %d</description>
			<pubDate>Sun, 14 Jun 2026 08:00:00 +0700</pubDate>
			<author>PV CafeBiz</author>
			<thumb>https://cdn.cafebiz.vn/img%d.jpg</thumb>
		</item>`, i+1, i+1, i+1, i+1, i+1)
	}
	return `<?xml version="1.0" encoding="UTF-8"?>
<rss version="2.0">
	<channel>
		<title>CafeBiz RSS</title>
		<link>https://cafebiz.vn</link>` + items + `
	</channel>
</rss>`
}
...
func TestLatestArticles(t *testing.T) {
	...
	if articles[0].Category != "home" {
		t.Errorf("category = %q, want home", articles[0].Category)
	}
}
...
func TestCategoryArticles(t *testing.T) {
	...
	c := newTestClient(srv)
	articles, err := c.CategoryArticles(context.Background(), "doanh-nhan", 10)
	if err != nil {
		t.Fatal(err)
	}
	if len(articles) != 3 {
	...
	if articles[0].Category != "doanh-nhan" {
		t.Errorf("category = %q, want doanh-nhan", articles[0].Category)
	}
}
...
func TestCategoryLimit(t *testing.T) {
	...
	c := newTestClient(srv)
	articles, err := c.CategoryArticles(context.Background(), "startup", 4)
	if err != nil {
		t.Fatal(err)
	}
	if len(articles) != 4 {
...
func TestExtractArticleID(t *testing.T) {
	cases := []struct{ url, want string }{
		{"https://cafebiz.vn/doanh-nhan-nao-do-190001.chn", "190001"},
		{"https://cafebiz.vn/startup/cong-ty-moi-1234567.chn", "1234567"},
		{"https://cafebiz.vn/no-id-here.chn", ""},
	}
