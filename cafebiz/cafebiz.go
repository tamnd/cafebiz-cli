// Package cafebiz is the library behind the cafebiz command line:
// the HTTP client, RSS feed parsing, and typed data models for CafeBiz
// (cafebiz.vn), Vietnam's leading business news portal (VCCorp).
//
// CafeBiz publishes per-category RSS 2.0 feeds. Article URLs embed a
// numeric ID: https://cafebiz.vn/{slug}-{id}.chn.
package cafebiz
...
const Host = "cafebiz.vn"

// baseURL is the site root.
const baseURL = "https://cafebiz.vn"

// DefaultUserAgent identifies this client to CafeBiz.
const DefaultUserAgent = "cafebiz-cli/0.1.0 (+https://github.com/tamnd/cafebiz-cli)"

// Categories lists the CafeBiz RSS feed category slugs.
...
// Client talks to CafeBiz RSS feeds over HTTP.
type Client struct {
	cfg  Config
	http *http.Client
	last time.Time
...
// Article is one CafeBiz article extracted from an RSS feed.
type Article struct {
...
// Category represents one CafeBiz RSS feed category.
...
func (c *Client) SearchArticles(ctx context.Context, query string, limit int) ([]*Article, error) {
	...
	for _, slug := range []string{"home", "doanh-nhan", "doanh-nghiep", "startup", "thi-truong"} {
		if len(out) >= limit {
			break
		}
		body, err := c.Get(ctx, c.rssURL(slug))
...
// ListCategories returns all known CafeBiz RSS feed categories.
...
// articleIDRE extracts the numeric ID from CafeBiz article URLs.
// Pattern: https://cafebiz.vn/{slug}-{id}.chn
