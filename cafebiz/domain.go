...
func (Domain) Info() kit.DomainInfo {
	return kit.DomainInfo{
		...
		Identity: kit.Identity{
			Binary: "cafebiz",
			Short:  "Read public CafeBiz (cafebiz.vn) business news articles.",
			Long: `Read public CafeBiz (cafebiz.vn) business news articles.

cafebiz reads from CafeBiz RSS feeds — no API key, no browser required.
...
// Classify turns a CafeBiz URL or numeric ID into (type, id).
func (Domain) Classify(input string) (uriType, id string, err error) {
	input = strings.TrimSpace(input)
	if input == "" {
		return "", "", errs.Usage("empty CafeBiz reference")
	}
	if strings.Contains(input, "cafebiz.vn/") {
